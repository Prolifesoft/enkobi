...
wp_enqueue_style('header-classic', THEME_DIR . '/assets/css/classic/classic-header.css');
            ...
            <div class="<?php echo esc_attr($headerWidth); ?>">
                <div class="row">
                    ...
                    <div class="header-right-panel clearfix col-md-10 col-sm-10 col-xs-12">
                        <?php
                        $archive_page   =   false;
                        if (is_search() || is_tax('listing-category') || is_tax('location')) {
                            $archive_page   =   true;
                        }
                        ?>
                        <?php if ($headerSrch == 1 && ($listingpro_options['listing_style'] != 4 || !$archive_page)) { ?>
                            <?php if (!is_front_page() && !is_page_template('template-dashboard.php')) { ?>
                                <?php if ((isset($lp_what_field_switcher) && $lp_what_field_switcher == 0) || (isset($search_loc_field_hide) && $search_loc_field_hide == 0)) { ?>
                        ...
                        <?php
                                $add_class_header_mobile = '';
                                $add_class_header = '';
                                if (is_front_page()) {
                                    $add_class_header_mobile = "col-xs-3";
                                    $add_class_header = "col-md-9 col-xs-9";
                                }else{
                                    $add_class_header_mobile = "col-xs-6";
                                    $add_class_header = "col-md-6 col-xs-6";
                                } ?>
                        <div class="<?php echo $add_class_header_mobile; ?> mobile-nav-icon">
                        ...
                        <div class="<?php echo $add_class_header; ?> lp-menu-container clearfix pull-right">
                            <div class="pull-right contentlogin-flex">
                                        ...
                                        <div class="lp-add-listing-btn">
                                            <ul>
                                                <li>
                                                    ...
                                                    <a href="<?php echo esc_url($addURL); ?>" class="header-list-icon-st6">
								...
								$logedinClass  = ' ';
								if(is_user_logged_in()){
									$logedinClass  = 'user-login';
									$logedindown  = '<i class="fa-solid fa-chevron-down"></i>';
								}
                                ?>
                                <div class="lp-joinus-icon-outer">
                                    <div class="lp-joinus-icon <?php echo $logedinClass; ?>">
                                        <?php get_template_part('templates/join-now'); ?>
										<?php echo $logedindown; ?>
                                    </div>
                                </div>
                            </div>
