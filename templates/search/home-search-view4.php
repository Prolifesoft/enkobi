...
$lp_what_field_switcher = $listingpro_options['lp_what_field_switcher'] ? $listingpro_options['lp_what_field_switcher'] : 0;
$search_loc_field_hide = $listingpro_options['lp_location_loc_switcher'] ? $listingpro_options['lp_location_loc_switcher'] : 0;
$hideWhereClass = '';
$searchHide = '';
...
$locOption = '';
if ($search_loc_option == 'yes' && (is_front_page() || is_page($app_view_home))) {
    $locOption = 'yes';
...
?>
    <div class="lp-search-bar classic-search clearfix">
        <form autocomplete="off" class="form-inline" action="<?php echo home_url(); ?>" method="get"
                ...
                <div class="form-group lp-suggested-search <?php echo esc_attr($hideWhereClass); ?> <?php echo esc_attr($hideWhatClass); ?>">
                    ...
                    <div class="pos-relative">
                        <div class="what-placeholder pos-relative" data-holder="">
                            ...
                            <i class="down-search-q fa fa-angle-down" aria-hidden="true"></i>
                                ...
                                if (empty($default_search_cats)) {
                                    ...
                                    if (!empty($listCatTerms) && !is_wp_error($listCatTerms)) {
                                        foreach ($listCatTerms as $term) {
                                            ...
                                            $childTerms = get_terms($child_args);
                                            if (!empty($childTerms) && !is_wp_error($childTerms)) {
                                                foreach ($childTerms as $childTerm) {
                                                    $childIcon = listingpro_get_term_meta($childTerm->term_id, 'lp_category_image');
                                                    if (!empty($childIcon)) {
                                                        if (hasFontAwesomeIconClass($childIcon)) {
                                                            $childIcon = '<i class="icon icons-search-cat ' . $childIcon . '"></i>';
                                                        } else {
                                                            $childIcon = '<img alt="image" src="' . $childIcon . '" />';
                                                        }
                                                    }
                                                    echo '<li class="lp-wrap-cats" data-catid="' . $childTerm->term_id . '">' . $childIcon . '<span class="lp-s-cat">-- ' . $childTerm->name . '</span></li>';
                                                    $defaultCats .= '<li class="lp-wrap-cats" data-catid="' . $childTerm->term_id . '">' . $childIcon . '<span class="lp-s-cat">' . $childTerm->name . '</span></li>';
                                                }
                                            }
                                        }
                                    }
            ...
            if ((isset($lp_what_field_switcher) && $lp_what_field_switcher == 0) || (isset($search_loc_field_hide) && $search_loc_field_hide == 0)) {
                ...
                <div class="form-group pull-right <?php echo esc_attr($searchHide); ?>">
                    <div class="lp-search-bar-right">
                        ...
                        <input value="<?php echo esc_html__('Search', 'listingpro'); ?>" class="lp-search-btn"
                            type="submit">
