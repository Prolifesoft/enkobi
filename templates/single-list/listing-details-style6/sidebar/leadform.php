...
$claimed_section = listing_get_metabox_by_ID('claimed_section', get_the_ID());
...
if ($lead_form_customizer == 'yes') {
    ...
    if (!function_exists('listingpro_lead_submited_data')) {
        function listingpro_lead_submited_data($user_ids)
        {
?>
            <ul>
                <?php
                $listings_count =   count_user_posts_by_status('listing', 'publish', $user_ids);
                ?>
                <li class="banner-list">
                    <i class="fa fa-list"></i>
                    <?php echo esc_attr($listings_count); ?>
                </li>
                <?php
                $photos_count =   count_user_posts_by_status('attachment', 'inherit', $user_ids);
                ?>
                <li class="banner-photos">
                    <i class="fa fa-camera"></i>
                    <?php echo esc_attr($photos_count); ?>
                </li>
                <?php
                $reviews_args   =   array(
                ...
                <li class="banner-ratings">
                    ...
                    <?php echo esc_attr($review_count); ?>
                </li>
            </ul>
        <?php
        }
        ...
        <div class="widget-box business-contact classic-business-lead-form lp-lead-form-st">
            <div class="user_texts">
                ...
                <div class="author-socials">
                    <div class="status">
                        <h5 class="online"><a href="<?php echo get_author_posts_url(get_the_author_meta('ID')); ?>"><?php echo get_the_author_meta('display_name'); ?></a></h5>
                        <p class="user-joining-status">
                            <?php if ($user_data) {
                                $joined_date = $user_data->user_registered;
                                $formatted_date = date('M Y', strtotime($joined_date));
                                echo 'Joined in ' . $formatted_date;
                            } ?>
                        </p>
                            <?php echo listingpro_lead_submited_data($user_ids); ?>
                    </div>
                    <a class="visit-profile-status" href="<?php echo get_author_posts_url(get_the_author_meta('ID')); ?>"><?php esc_html_e('Visit Profile', 'listingpro'); ?></a>
                </div>
            </div>
            <div class="clearfix"></div>
            <div class="contact-form quickform">
                <form data-lp-recaptcha="<?php echo wp_kses_post($enableCaptcha); ?>" data-lp-recaptcha-sitekey="<?php echo wp_kses_post($gSiteKey); ?>" class="form-horizontal hidding-form-feilds margin-top-20" method="post" id="contactOwner">
                    <?php
